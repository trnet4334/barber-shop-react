...
const Pricing = () => {
  ...
  const serviceItems = price.map((service) =>
    <ul
      className="list-none text-left w-[85vw] lg:w-[40vw] mb-5"
      key={service.translation}
    >
      <h4 className="flex items-end text-secondary-dark text-xl md:text-2xl mb-4 pb-3 border-b-[0.1px] border-gray-600">
          ...
          >
            <div className="flex justify-between items-center mb-1">
              ...
              <div className="inline-flex">
                <div className="border-b-2 border-dotted border-secondary-dark w-12 md:w-24 -translate-y-2.5 mr-1"/>
                <span className="text-sm md:text-base text-secondary-dark">${item.price}</span>
              </div>
            </div>
  ...
  return (
    <section className="bg-primary max-w-screen h-auto min-h-screen font-noto px-10 py-12 overflow-hidden">
      <div className="w-full flex mb-14 items-center justify-center">
        <h2 className="text-3xl md:text-4xl text-secondary-mild text-center mr-3">價格說明</h2>
        <i className="fas fa-cut fa-2x text-secondary-mild"/>
      </div>
